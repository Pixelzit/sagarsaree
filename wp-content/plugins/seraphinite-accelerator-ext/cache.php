...
function _OutImage( $siteId, $sett, $file, $nonce, $aiFileId = null )
{
	...
	if( (isset($sett[ 'hdrTrace' ])?$sett[ 'hdrTrace' ]:null) )
		@header( 'X-Seraph-Accel-Cache: 2.25.2; state=' . ( $mimeType == Fs::MimeTypeDef ? 'original' : 'preoptimized' ) . '; redir=alt;' . ( $aSkip ? ( '; skip=' . @json_encode( $aSkip ) ) : '' ) . ( $mimeType != Fs::MimeTypeDef ? ( '; sizeOrig=' . @filesize( $fileOrig ) ) : '' ) );
...
function _Process( $sites, $args )
{
	...
	if( isset( $_REQUEST[ 'seraph_accel_gi' ] ) )
	{
		...
		_OutImage( $seraph_accel_g_siteId, $sett, $file, Gen::SanitizeId( (isset($_REQUEST[ 'n' ])?$_REQUEST[ 'n' ]:null) ), Gen::SanitizeId( (isset($_REQUEST[ 'ai' ])?$_REQUEST[ 'ai' ]:null) ) );
		exit();
	}

	if( isset( $_REQUEST[ 'seraph_accel_gci' ] ) )
	{
		_OutImage( $seraph_accel_g_siteId, $sett, $_REQUEST[ 'seraph_accel_gci' ], Gen::SanitizeId( (isset($_REQUEST[ 'n' ])?$_REQUEST[ 'n' ]:null) ) );
		exit();
	}
...
function _ProcessOutHdrTrace( $sett, $bHdr, $bLog, $state, $data = null, $dscFile = null )
{
	...
	if( $bHdr )
		@header( 'X-Seraph-Accel-Cache: 2.25.2;' . $debugInfo );
...
function GetCacheViewId( $ctxCache, $settCache, $userAgent, $path, $pathOrig, &$args )
{
	...
	if( (isset($settCache[ 'normAgent' ])?$settCache[ 'normAgent' ]:null) )
	{
		...
		$_SERVER[ 'HTTP_USER_AGENT' ] = 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.25.2';
	}
