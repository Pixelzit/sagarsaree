...
class StyleProcessor
{
	...
	function AdjustCont( $extract, &$aCritFonts, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, &$cont, $isInlined )
	{
		if( isset( $ctxProcess[ 'lrnDsc' ] ) )
		{
			...
			$aInfo = (isset($this -> _aAdjustContCache[ $contHash ])?$this -> _aAdjustContCache[ $contHash ]:null);
			if( $aInfo )
			{
				$res = (isset($aInfo[ 'c' ])?$aInfo[ 'c' ]:null);
				if( is_array( $res ) )
				{
					$ok = true;
					$aDeps = array();
	...
	{
		$a[ $sel ] = $scope | (isset($a[ $sel ])?$a[ $sel ]:0);
	}
	...
	static function AdjustRuleSet( $ruleSet, &$aDepFonts, $ctxItems, $doc, &$ctxProcess, $settCache, $settCss, $settImg, $settCdn, $srcInfo, $src, $isInlined, $imgSzAlternatives = null )
	{
		...
		foreach( $ruleSet -> getRules( 'background-image' ) as $rule )
		{
			...
			foreach( $urls as $url )
			{
				...
				{
					if( Gen::HrFail( Images_ProcessSrc_SizeAlternativesEx( $imgSzAlternatives, $ctxProcess, $bgImgSrc, $settCache, $settImg, $settCdn, true, $rule -> getIsImportant() ) ) )
						return( false );
	...
	static function keepLrnNeededData( &$datasDel, &$lrnsGlobDel, $dsc, $dataPath )
	{
		...
		if( ( (isset($dsc[ 'v' ])?$dsc[ 'v' ]:null) < 2 ) && ( $id = Gen::GetArrField( $dsc, array( 'css', 'xslb' ) ) ) )
		{
