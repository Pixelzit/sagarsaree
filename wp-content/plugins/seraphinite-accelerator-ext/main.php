...
{
	add_menu_page( Plugin::GetPluginString( 'TitleLong' ), Plugin::GetNavMenuTitle(), 'manage_options', 'seraph_accel_manage',																		$accepted ? 'seraph_accel\\_ManagePage' : 'seraph_accel\\Plugin::OutputNotAcceptedPageContent', Plugin::FileUri( 'icon.png?v=2.25.2', __FILE__ ) );
				...
				echo( Ui::TagOpen( 'div', array( 'class' => 'blck' ) ) );
				{
					...
					$o = Ui::TagOpen( 'div' );
					{
						...
						if( $adminMsModes[ 'local' ] )
						{
							...
							$o .= Ui::Button( esc_html_x( 'Pause', 'admin.Manage_Queue', 'seraphinite-accelerator' ), false, null, null, 'button', array( 'class' => array( 'pause', 'ctlSpaceAfter', 'ctlVaMiddle' ), 'style' => array( 'min-width' => '7em', 'display' => !$bPaused ? null : 'none' ), 'onclick' => 'seraph_accel.Manager._int.OnQueuePause(this,true,"' . wp_create_nonce( 'pause' ) . '");return false;' ) );
...
function GetHostingBannerContent()
{
	...
	$urlLogoImg = add_query_arg( array( 'v' => '2.25.2' ), Plugin::FileUri( 'Images/hosting-icon-banner.svg', __FILE__ ) );
	...
	if( $nMaxItems )
	{
		...
		foreach( $items as $itemState => $itemsPerState )
		{
			foreach( $itemsPerState as $item )
			{
				...
				$initiatorPrior = (isset($data[ 'p' ])?$data[ 'p' ]:null);
				if( $initiatorPrior === null )
					$initiatorPrior = (isset($item[ 'p' ])?$item[ 'p' ]:null);
				...
				$itemType = '';
				switch( ( int )(isset($item[ 'tp' ])?$item[ 'tp' ]:0) )
				{
				...
				$res .= Ui::Tag( 'tr',
					Ui::Tag( 'td', Ui::LogItem( $iconClr, Ui::Link( (isset($data[ 'u' ])?$data[ 'u' ]:null), ( int )(isset($item[ 'tp' ])?$item[ 'tp' ]:0) === 0 ? (isset($data[ 'u' ])?$data[ 'u' ]:null) : null, true ), false ), array( 'class' => 'ctlNoWrap cellSpaceAfter' ) ) .
					Ui::Tag( 'td', $itemType, array( 'class' => 'ctlNoWrap cellSpaceAfter' ) ) .
					Ui::Tag( 'td', $initiator, array( 'class' => 'ctlNoWrap cellSpaceAfter' ) ) .
					Ui::Tag( 'td', GetViewDisplayName( (isset($data[ 'v' ])?$data[ 'v' ]:null), $isViewsEnabled, ( int )(isset($item[ 'tp' ])?$item[ 'tp' ]:0) ), array( 'class' => 'ctlNoWrap cellSpaceAfter' ) ) .
...
function OnAdminApi_QueuePause( $args )
{
	if( !current_user_can( 'manage_options' ) )
		return( Gen::E_ACCESS_DENIED );
	if( !wp_verify_nonce( (isset($args[ '_wpnonce' ])?$args[ '_wpnonce' ]:''), 'pause' ) )
		return( Gen::E_CONTEXT_EXPIRED );

	if( (isset($args[ 'v' ])?$args[ 'v' ]:null) )
