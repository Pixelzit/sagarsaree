...
function _Images_ProcessSrc_CopyImageToHost( &$ctxProcess, $imgSrc, $imgCont, $settCache, $settImg )
{
	...
	Images_ProcessSrc_ConvertAll( $settImg, $imgCont, $file, Images_ProcessSrcEx_FileMTime( $file ) );
	return( true );
}
...
function _Images_ProcessSrc_ConvertEx( $type, $typeIdx, $settImg, $data, $file, $fileCnv, $fileTime, $fileTimeCnv, $aTypeFrom, &$sizeCheck, $postpone )
{
	...
	if( ( $hr != Gen::S_OK && $hr != Gen::S_IO_PENDING ) || $status )
	{
		@unlink( $fileCnv );
		if( $status )
...
function Images_ProcessSrc_ConvertAll( $settImg, $imgSrcOrCont, $file, $fileTime, $postpone = null )
{
	...
	foreach( array( 'webp','avif' ) as $typeIdx => $type )
	{
		...
		if( _Images_ProcessSrc_ConvertEx( $type, $typeIdx, $settImg, $imgSrcOrCont, $file, $fileCnv, $fileTime, $fileTimeCnv, $aTypeFrom, $sizeCheck, $postpone ) == Gen::S_IO_PENDING )
			$bNeedPostpone = true;
	...
	if( $bNeedPostpone )
		CachePostPrepareObjEx( 10, $file, $seraph_accel_g_siteId, (isset($seraph_accel_g_prepPrms[ 'p' ])?$seraph_accel_g_prepPrms[ 'p' ]:10) );
}
...
function Images_ProcessSrcEx( &$ctxProcess, $imgSrc, $settCache, $settImg )
{
	...
	Images_ProcessSrc_ConvertAll( $settImg, $imgSrc, $file, $fileTime );
...
function Images_ProcessSrc_SizeAlternativesEx( $imgSzAlternatives, &$ctxProcess, $imgSrc, $settCache, $settImg, $settCdn, $bCrop = false, $isImportant = false, $idAiOnly = null, $postpone = null )
{
	...
	if( $hr == Gen::S_IO_PENDING && ( $file = $imgSrc -> GetFile() ) && !Gen::GetArrField( $settImg, array( 'szAdaptOnDemand' ), false ) )
		CachePostPrepareObjEx( 20, $file, $seraph_accel_g_siteId, (isset($seraph_accel_g_prepPrms[ 'p' ])?$seraph_accel_g_prepPrms[ 'p' ]:10), $idAiOnly ? array( 'ai' => $idAiOnly ) : array() );
...
function Images_ProcessSrc_SizeAlternativesExEx( $imgSzAlternatives, &$ctxProcess, $imgSrc, $settCache, $settImg, $settCdn, $bCrop = false, $isImportant = false, $idAiOnly = null, $postpone = null )
{
	...
	foreach( array( $info[ 'cx' ], 				1920,	1366,	992,	768,	480,	360,	120	 ) as $i => $cx )
	{
		...
		foreach( ( $bCrop ? array( $cx, 	1920,	1366,	992,	768,	480,	360				 ) : array( $cx ) ) as $cxCrop )
		{
			...
			else if( $infoCacheVal = (isset($infoCache[ $idAi ])?$infoCache[ $idAi ]:null) )
			{
			...
			if( !$infoCacheVal )
			{
				if( !$postpone )
				{
					...
					if( $seraph_accel_g_prepPrms )
						ProcessCtlData_Update( (isset($seraph_accel_g_prepPrms[ 'pc' ])?$seraph_accel_g_prepPrms[ 'pc' ]:null), array( 'stageDsc' => LocId::Pack( 'ImgAdaptFile_%1$s%2$s', null, array( $imgSrc -> GetDisplayFile(), $idAi ) ) ) );
			...
			if( $file )
			{
				Images_ProcessSrc_ConvertAll( $settImg, null, $file, Images_ProcessSrcEx_FileMTime( $file ) );
				...
				if( $bSepStg )
				{
					if( $imgSzAlternatives -> srcTpl === null )
					{
						...
						{
							$fileOrigRelPath = substr( $imgSrc -> GetFile(), strlen( $ctxProcess[ 'siteRootPath' ] ) + 1 );
...
function Images_Process( &$ctxProcess, $doc, $settCache, $settImg, $settCdn )
{
	...
	foreach( $items as $item )
	{
		...
		if( $attr )
		{
			...
			if( (isset($settImg[ 'szAdaptImg' ])?$settImg[ 'szAdaptImg' ]:null) )
			{
...
function Images_ProcessSrcSizeAlternatives_Cache_Cleanup( $dataPath, $tm, $cbIsAborted )
{
...

	{
		...
		foreach( $aCache as $imgStgId => $a )
		{
			...
			if( ( int )(isset($a[ 't' ])?$a[ 't' ]:null) < $ctx -> tm )
				$aDel[] = $imgStgId;
		}
