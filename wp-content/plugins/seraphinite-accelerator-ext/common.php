	...
	if( $itemType == 0 )
	{
...

		{
			...
			if( $asyncMode == 're' || $asyncMode == 're_r' )
				$ctx -> requestRes = PluginRe::MakeRequest( 'GET', $ctx -> url, $ctx -> hdrsForRequest, 30, false );
			else
...
function CacheProcessItem_InitExtCron()
{
	...
	while( $bLoop )
	{
		foreach( GetSiteIds() as $siteId )
		{
			...
			if( $lock -> Acquire() )
			{
				...
				{
					if( (isset($itemTmp[ 'stExt' ])?$itemTmp[ 'stExt' ]:null) )
						continue;
	...
	$data = Gen::GetArrField( $item ? Gen::Unserialize( (isset($item[ 'd' ])?$item[ 'd' ]:null) ) : null, array( '' ), array() );
	if( !$data )
	{
		if( $mode == 'O' )
			exit;
	...
	$aUrlParts = Net::UrlParse( $ctx -> url );
	...
	foreach( $ctx -> hdrsForRequest as $hdrKey => $hdrVal )
	{
