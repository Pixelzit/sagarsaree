...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.25.2' || $lastCheckPackage !== 'Full' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '2.25.2' && !isset( $state[ 'changeVerCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '2.25.2' )
				return( Gen::S_FALSE );
...
class Plugin
{
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.25.2' ), '2.25.2' ) > 0 )
					{
	...
	static private function _AsyncTasksProcess()
	{
		...
		{
			if( self::_AsyncTasks_GetMode() == 'ec' )
				self::_AsyncTasksProcessMgr( false, function( $dataItem ) { return( !!(isset($dataItem[ 'f' ])?$dataItem[ 'f' ]:null) ); }, true, 15, 120, Gen::GetCurRequestTime() );
			else
		...
		{
			if( self::_AsyncTasks_GetMode() == 'ec' )
				self::_AsyncTasksProcessMgr( false, function( $dataItem ) { return( !(isset($dataItem[ 'f' ])?$dataItem[ 'f' ]:null) ); }, true, false, 60, Gen::GetCurRequestTime() );
	...
	static private function _AsyncTasksProcessMgr( $bMt = true, $cbFilter = null, $bYeld = true, $tmMaxRunWait = 30, $tmMaxRun = 60, $tmStart = null )
	{
		...
		for( ;; )
		{
			...
			if( $dataItem )
			{
				if( !$bMt || (isset($dataItem[ 'f' ])?$dataItem[ 'f' ]:null) )
					self::_AsyncTaskRun( $dataItem );
				else
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '2.25.2' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '2.25.2' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '2.25.2' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
	...
	static private function _PrevVer_Check()
	{
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '2.25.2' );
			if( $verTo < $verFrom )
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.25.2.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
		...
		if( ( $switch || version_compare( $verLast, '2.25.2' ) > 0 ) && !isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
		...
		if( isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
			...
			if( !$switch && version_compare( $verLast, '2.25.2' ) > 0 )
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '2.25.2' );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '2.25.2' );
			if( $id == 'Gen' )
	...
	static function _on_admin_action_act()
	{
		...
		case 'settExport':
			if( !current_user_can( 'manage_options' ) )
				wp_die( '', 403 );
			else if( !wp_verify_nonce( (isset($_REQUEST[ '_wpnonce' ])?$_REQUEST[ '_wpnonce' ]:''), 'export' ) )
				wp_die( '', 403 );
				...
				<script>
					(function()
					{
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "2.25.2" ) );
...
class PluginRe
{
	...
	static function GetOpCacheDir()
	{
		...
		if( !$sPhpLauncher )
			return( false );
		return( self::_GetOpCacheDir( $sPhpLauncher, Gen::GetTempDirEx() ) );
	}
