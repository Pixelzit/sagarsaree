...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	{
		if( !headers_sent() )
			header( 'X-Seraph-Accel-Content: 2.25.2; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
...
function ContentProcess( &$ctxProcess, $sett, $settCache, $settContPr, $buffer, &$skipStatus )
{
	...
	if( $ctxProcess[ 'mode' ] == 'full' )
	{
		...
		if( GetContentProcessorForce( $sett ) && (isset($_REQUEST[ 'd' ])?$_REQUEST[ 'd' ]:null) == 'learn' )
		{
			...
			if( isset( $contGrpResTmp[ 2 ] ) )
			{
				...
				$contSkeletonHash = (isset($contGrpTmp[ 'sklSrch' ])?$contGrpTmp[ 'sklSrch' ]:null) === 'a' ? $grpVariationDataIdTmp : GetContSkeleton( $ctxProcess[ 'ndBody' ], Gen::GetArrField( $contGrpTmp, array( 'sklExcl' ), array() ), $sklCssSelExcl );

				$out .= Ui::Tag( 'h2', 'ID' ) . Ui::Tag( 'p', $contGrpResTmp[ 2 ][ 1 ] . '/' . $contSkeletonHash );
				$out .= Ui::Tag( 'h2', 'Tree' ) . Ui::Tag( 'pre', GetContSkeleton( $ctxProcess[ 'ndBody' ], Gen::GetArrField( $contGrpTmp, array( 'sklExcl' ), array() ), $sklCssSelExcl, 'text', (isset($contGrpTmp[ 'sklSrch' ])?$contGrpTmp[ 'sklSrch' ]:null) === 'a' ) );
			}
			else
				$out .= 'Not defined';
		...
		if( ( $seraph_accel_g_prepPrms !== null  ) && isset( $contGrpRes[ 2 ] ) )
		{
			...
			if( (isset($contGrp[ 'sklSrch' ])?$contGrp[ 'sklSrch' ]:null) === 'a' )
			{
				$contSkeletonHash = $grpVariationDataId;
				$aSkeletonAggr = GetContSkeleton( $ctxProcess[ 'ndBody' ], Gen::GetArrField( $contGrp, array( 'sklExcl' ), array() ), (isset($ctxProcess[ 'sklCssSelExcl' ])?$ctxProcess[ 'sklCssSelExcl' ]:null), 'tree', true );

			}
			else
				$contSkeletonHash = GetContSkeleton( $ctxProcess[ 'ndBody' ], Gen::GetArrField( $contGrp, array( 'sklExcl' ), array() ), (isset($ctxProcess[ 'sklCssSelExcl' ])?$ctxProcess[ 'sklCssSelExcl' ]:null), 'hash', false, (isset($ctxProcess[ 'docSkeleton' ])?$ctxProcess[ 'docSkeleton' ]:null) );
			...
			if( isset( $seraph_accel_g_prepPrms[ 'lrn' ] ) )
			{
				...
				if( $aSkeletonAggr !== null )
				{
					...
					GetContSkeleton_GenNodesFromAgg( (isset($ctxProcess[ 'docSkeleton' ])?$ctxProcess[ 'docSkeleton' ]:null), $aSkeletonAggr );
			...
			else if( Learn_Init( $ctxProcess, $settHash, $aSkeletonAggr ) )
			{
				if( $aSkeletonAggr !== null )
				{
					$aSkeletonAggr = ( array )(isset($ctxProcess[ 'lrnDsc' ][ 's' ])?$ctxProcess[ 'lrnDsc' ][ 's' ]:null);
					GetContSkeleton_GenNodesFromAgg( (isset($ctxProcess[ 'docSkeleton' ])?$ctxProcess[ 'docSkeleton' ]:null), $aSkeletonAggr );
				}
			}
			else
		...
		if( $aCustStyles = Gen::GetArrField( $settCss, array( 'custom' ), array() ) )
		{
			...
			foreach( $aCustStyles as $custStyle )
			{
				if( !(isset($custStyle[ 'enable' ])?$custStyle[ 'enable' ]:null) )
					continue;
				if( !(isset($custStyle[ 'noJsDl' ])?$custStyle[ 'noJsDl' ]:null) && !$ctxProcess[ 'isJsDelayed' ] )
					continue;
	...
	if( $ctxProcess[ 'mode' ] == 'full' )
	{
		...
		if( (isset($ctxProcess[ 'lazyload' ])?$ctxProcess[ 'lazyload' ]:null) || (isset($ctxProcess[ 'imgAdaptive' ])?$ctxProcess[ 'imgAdaptive' ]:null) )
		{
...

			{
				...
				if( (isset($ctxProcess[ 'lazyloadBjs' ])?$ctxProcess[ 'lazyloadBjs' ]:null) )
				{
		...
		if( (isset($ctxProcess[ 'lazyVid' ])?$ctxProcess[ 'lazyVid' ]:null) )
		{
...
function _GetContSkeletonHash_Enum( &$aParentUniqueItems, $itemParent, $aExcl, $bAgg = false )
{
	...
	foreach( $itemParent -> childNodes as $item )
	{
		...
		$aUniqueItems = array();
			...
			_GetContSkeletonHash_Enum( $aUniqueItems, $item, $aExcl, $bAgg );
...
function _GetContSkeletonHash_EnumUniqueItems( &$contTpl, $docTpl, $itemParentTpl, &$aParentUniqueItems, $mode, $bAgg, $level = 0 )
{
	...
	foreach( $aParentUniqueItems as $contItemTpl => &$aUniqueItems )
	{
		if( $bAgg )
		{
			...
			if( (isset($aUniqueItems[ 'c' ])?$aUniqueItems[ 'c' ]:null) )
			{
			...
			if( (isset($aUniqueItems[ 'i' ])?$aUniqueItems[ 'i' ]:null) )
			{
...
function _GetContSkeleton_GenNodesFromAgg( $docTpl, $itemParentTpl, &$aParentUniqueItems )
{
	...
	foreach( $aParentUniqueItems as $contItemTpl => &$aUniqueItems )
	{
		...
		$contItemTplIds = (isset($aUniqueItems[ 'i' ])?$aUniqueItems[ 'i' ]:null);
		if( !$contItemTplIds )
...
function GetContSkeleton( $ndBody, $excls, $exclsCssSel, $mode = 'hash', $bAgg = false, $docTpl = null )
{
	...
	$aUniqueItems = array();
	_GetContSkeletonHash_Enum( $aUniqueItems, $ndBody -> parentNode -> parentNode, $aExcl, $bAgg );
	...
	if( $exclItems === null )
	{
		...
		if( $excls )
		{
			...
			foreach( $excls as $exclItemPath )
			{
				...
				{
					if( (isset($ctxProcess[ 'isJsDelayed' ])?$ctxProcess[ 'isJsDelayed' ]:null) )
						$mode = 'ajs';
